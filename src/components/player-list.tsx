  ...
  return (
    <div
      className={clsx(styles.player, {
        ...
        [styles._looser]: board.status === "roundOver" && player.id === board.turn,
    ...
    >
      <div className={styles.name}>{player.name || player.id}</div>
...
export function PlayerList() {
  ...
  const positions = useMemo(() => {
    ...
    const firstPlayerIdx = board.players.findIndex((p) => p.id === board.me);
  ...
  return (
    <div className={styles.wrapper}>
        ...
        {board.players.map((p) => {
            return (
            <Player key={p.id} player={p} position={positions[p.id]}></Player>
            );
        })}
        </div>
    </div>
